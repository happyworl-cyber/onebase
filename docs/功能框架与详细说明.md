...
> 说明：本文以当前代码为准。仓库目录、Rust crate 与产品名均统一为 **PlaneOS**。

## 1. 产品定位

PlaneOS 是面向私有化交付场景的数据与自动化平台。其核心目标是将 PostgreSQL 数据库、API、权限、工作流、多数据源和 AI/MCP 能力统一到一个可部署、可审计、可授权的平台中。
...
`src/sso.rs` 和 `src/sso_handlers.rs` 实现 PlaneOS 登录 SSO，支持预设 Provider 和通用 OIDC。
...
`src/idp_oidc.rs` 和 `src/idp_handlers.rs` 允许项目将 PlaneOS 作为 OIDC Provider 对外提供身份能力。
...
`mcp-server` 是 Node.js 服务，使用 `obp_` 平台令牌调用 PlaneOS HTTP API。
