...
用户要维护一批经常改动的 XML（及少量其它文本）文件。改动在 PlaneOS **之外**完成（编辑器或其它系统覆盖写入对象存储）。PlaneOS 只做查看入口：打开时读到的是对象存储里该 key 的**当前内容**。改的人和看的人可以不是同一人。不需要变更通知、版本历史、diff。

PlaneOS 已有对象存储连接（MinIO / COS / OSS / GCS）和 `list` / `get` / `presign`。现有「集成 → 对象存储」页是 admin 用的连接控制台（JSON exec），普通成员进不去，也不适合当日常打开 XML 的界面。
