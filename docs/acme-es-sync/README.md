...
把 `gamesq.article` / `gamesq.project_list` / `gamesq.sq_users` 三张表周期性同步到 Elasticsearch，再让业务通过 HTTP 关键词检索——**全程不修改 planeos 代码、不部署外部脚本**。
...
flowchart LR
  ...
  FN -->|"POST /api/es-app/:idx/bulk<br/>Authorization: ApiKey obes_es_*<br/>via http 扩展"| CR["planeos<br/>应用 API"]
  CR -->|"_bulk NDJSON"| ES[("Elasticsearch")]
  FE["前端 / 业务后端"] -->|"POST /api/es-app/:idx/search<br/>或 /api/es/* 反代"| CR
```

**关键点**：ES 的真实 URL / 账密**只配在 planeos【ES 反向代理】里一份**；PG 这边只需要知道 planeos 在哪、以及一个 `obes_es_*` token。同步、删除、查询都走 planeos 应用 API，token 自带 method/index 白名单审计。

---

## 第 0 步：装 `http` 扩展（一次性，DBA 做）

> 整个方案唯一需要走 DBA / RDS 控制台的步骤。planeos SQL 编辑器里跑的角色通常不是 superuser，没法自助 `CREATE EXTENSION`，所以**必须先在 DBA 这边做好**。
...
校验（在 planeos SQL 编辑器里跑这一条，确认装好即可，schema 不要求必须是 `public`）：
...
## 第 1 步：在 planeos【ES 反向代理】里申请一个 token

1. planeos 控制台 → 【ES 反向代理】→ 选既有的"测试ES"连接（截图里那条）→ 切到 `代理 Token` 标签 → 新建 token。
2. 配置：
...
> 这个 token 只给"PG 调 planeos"用，与给前端用的 token 是分开的两份（前端的可以只给 `GET`/`POST`，indices 限定一样）。

## 第 2 步：apply `install.sql`

打开 planeos 控制台 → 左侧【SQL 查询】→ **左上角连接选 `acme_test`（库 supabase, schema gamesq）**，把 `docs/acme-es-sync/install.sql` 整份复制进去，点执行。
...
| 对象 | 说明 |
| --- | --- |
| `_es_sync_config` | 设置表（planeos URL / token / 索引名 / batch） |
| `_es_setting / _strip_html / _parse_tac / _parse_description` | 工具 |
| `_es_call(method, path, body)` | planeos HTTP 客户端（自动加 `Authorization: ApiKey ...`） |
...
```sql
-- planeos HTTP 入口。注意必须是 PG 这台机器能访问到的地址：
--   · 单机部署：http://127.0.0.1:3006
--   · 远端 RDS + 公网 planeos：填 planeos 的公网域名 / 反代地址
UPDATE gamesq._es_sync_config SET value = 'http://127.0.0.1:3006' WHERE key = 'planeos_base_url';

-- 第 1 步拿到的 token
UPDATE gamesq._es_sync_config SET value = 'obes_es_粘贴你拿到的真实 token' WHERE key = 'planeos_es_token';
...
> 这张表只有 schema 内权限够的角色能读。`planeos_es_token` 还是会以明文存在于 `_es_sync_config.value` —— 这是必须的（PG 没有内置 secrets 管理），ES 真正的账密在 planeos 那边、不在这里。

## 第 4 步：在 SQL 控制台验证一次

> ⚠️ **planeos SQL 编辑器有 30s 查询超时**（后端为防误操作写死，UI 改不了）。下面给的命令都是按"≤30s 能跑完"挑的；**真正的同步以及全量都必须走第 5 步的调度器**（调度器每个任务的 `timeout_secs` 独立配置，默认能跑几百秒以上）。
...
- 三条都 `status: 200` 说明 PG ↔ planeos ↔ ES 全链路通了，token 也对
...
`upsert_fail` / `delete_fail` 非 0 时，去 planeos【ES 反向代理 → 代理 Token】里看该 token 的最近调用日志，能定位是哪个 op 失败。
...
| `es_health()` 的 `elapsed_ms` 普遍 > 500ms | PG 到 planeos 网络远 | 把 planeos 部署得离 PG 近一点；或调小 `*_batch` 减少单次 op 数（默认 article=500/community=200/user=500） |
...
进 planeos【定时任务】→ 新建。**左上角连接确保选的是 `acme_test`**（决定任务的 `database_id`，rpc 在该库执行）。
...
同步好之后业务侧两种方式查 ES，**都走 planeos，不需要业务方知道 ES 的真实地址/密码**。
...
- 同步函数都长这样：分页扫源表 → `jsonb_agg` 拼一个 `[{action:"index", id:"...", doc:{...}}, ...]` 数组 → 调 `_es_bulk(idx, ops)`，后者 POST 给 `/api/es-app/{idx}/bulk`，planeos 帮转 ES `_bulk` NDJSON，返回 `{results:[{ok:bool,...}]}` 后我们就地 count。
...
- planeos 调度器多实例时不会重复跑同一条 task（`runner.rs` 的 claim 索引约束）；rpc 函数在 `database_id` 指向的租户库执行。
- `http` 扩展同步阻塞 PG backend，但我们一次 bulk 只发 ≤500 op、过 planeos（同机 / 内网），延迟通常 <200ms，对 PG 连接占用很短。
...
| 在 SQL 编辑器跑 `es_sync_all(60)` 报 `timeout of 30000ms exceeded` | planeos SQL 编辑器写死 30s 上限，不是脚本问题。换用第 4.2 节的 `es_health()` 验通路 + 第 4.3 节单实体小窗口验数据形状；真正运行去第 5 步配调度任务 |
| apply `install.sql` 时 `extension "http" must be installed by superuser` | RDS 上让 DBA 在控制台勾装；自建 PG 用 `postgres` 角色装 |
| `gamesq._es_sync_config.planeos_base_url 未配置` | 第 3 步的 UPDATE 没跑，或写到了别的库 |
| `gamesq._es_sync_config.planeos_es_token 未配置或仍是占位值` | 同上，token 还是 `obes_es_REPLACE_ME` |
| `bulk way_xxx failed: status=401 body={"error":"invalid token"}` | token 拼错；或者 planeos 那边把 token 删了；回第 1 步重新申请 |
| `bulk way_xxx failed: status=403` | token 的 `methods_allow` / `indices_allow` 不包含本次调用；去【ES 反向代理 → 代理 Token】扩白名单 |
| `bulk way_xxx failed: status=502/503/timeout` | planeos 不可达或挂了；检查 `planeos_base_url` 在 PG 这台机器上能不能 `curl` 通；公网 RDS + 内网 planeos 要走反代/公网入口 |
