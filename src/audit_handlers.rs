...
pub async fn list_audit_logs(
    ...
    if let Some(ref ids) = tenant_filter { query = query.bind(ids); }
    if let Some(uid) = params.user_id { query = query.bind(uid); }
    if let Some(ref act) = params.action { query = query.bind(act); }
    if let Some(ref res) = params.resource { query = query.bind(format!("%{}%", res)); }
    if let Some(ref sd) = params.start_date { query = query.bind(sd); }
    if let Some(ref ed) = params.end_date { query = query.bind(ed); }
    ...
    if let Some(ref ids) = tenant_filter { count_query = count_query.bind(ids); }
    if let Some(uid) = params.user_id { count_query = count_query.bind(uid); }
    if let Some(ref act) = params.action { count_query = count_query.bind(act); }
    if let Some(ref res) = params.resource { count_query = count_query.bind(format!("%{}%", res)); }
    if let Some(ref sd) = params.start_date { count_query = count_query.bind(sd); }
    if let Some(ref ed) = params.end_date { count_query = count_query.bind(ed); }
...
pub async fn list_slow_queries(
    ...
    let queries: Vec<serde_json::Value> = rows.iter().map(|r| {
        json!({
            "id": r.get::<i64, _>("id"),
