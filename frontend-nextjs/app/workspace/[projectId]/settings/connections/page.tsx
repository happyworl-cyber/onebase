...
export default function ConnectionsPage() {
  ...
  const t = useTranslations('wsConnections')
  ...
  const loadConnections = async () => {
    ...
    try {
      ...
      console.error(t('loadFailedLog'), err)
      alert(t('loadFailed', { msg: err.response?.data?.error || err.message }))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      setTestResult({
        success: false,
        message: err.response?.data?.error || err.message || t('testFail'),
      })
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(t('createFailedLog'), err)
      alert(t('createFailed', { msg: err.response?.data?.error || err.message }))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleUseConnection = async (databaseId: number) => {
    try {
      ...
      console.error(t('switchFailedLog'), err)
      alert(t('switchFailed', { msg: err.response?.data?.error || err.message }))
    }
  }
  ...
  const handleMoveConnection = async (index: number, direction: -1 | 1) => {
    ...
    try {
      ...
      alert(t('reorderFailed', { msg: err.response?.data?.error || err.message }))
      await loadConnections()
    }
  }
  ...
  const handleEditSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(t('updateFailed', { msg: err.response?.data?.error || err.message }))
    } finally {
      setEditSaving(false)
      setUpdatingId(null)
    }
    ...
    <div className="space-y-6">
        ...
        <div className="card p-4 bg-blue-50 border-blue-200">
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              <h3 className="text-sm font-semibold text-blue-900 mb-1">{t('currentConn')}</h3>
              <p className="text-sm text-blue-700">
                {t('projectLabel')}<span className="font-medium">{currentConnection.tenant_name}</span>
                {' / '}
                {t('connLabel')}<span className="font-medium">{currentConnection.connection_name}</span>
              </p>
              <p className="text-xs text-blue-600 mt-1">
                {currentConnection.db_host}:{currentConnection.db_port}/{currentConnection.db_name}
                {' · '}
                {t('poolCount', { n: currentConnection.max_connections ?? DEFAULT_TENANT_MAX_CONNECTIONS })}
                {' · '}
                {t('acquireTimeout', { n: currentConnection.connection_timeout ?? DEFAULT_TENANT_ACQUIRE_TIMEOUT_SECS })}
              </p>
            </div>
          </div>
        </div>
      ...
      {showForm && (
        <div className="card p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('addNewConn')}</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* 角色选择 */}
            <div className="flex items-center space-x-6 p-3 bg-gray-50 rounded-lg">
              <span className="text-sm font-medium text-gray-700">{t('connRole')}</span>
              <label className="flex items-center space-x-2 cursor-pointer">
                ...
                <span className="text-sm text-gray-700">
                  <i className="fas fa-crown text-yellow-500 mr-1"></i>{t('rolePrimary')}
                </span>
              </label>
              <label className="flex items-center space-x-2 cursor-pointer">
                <input
                ...
                />
                <span className="text-sm text-gray-700">
                  <i className="fas fa-copy text-gray-500 mr-1"></i>{t('roleReplica')}
                </span>
              </label>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('fConnName')}</label>
                <input
                  type="text"
                  value={formData.connection_name}
                  onChange={(e) => setFormData({ ...formData, connection_name: e.target.value })}
                  className="input-base w-full"
                  placeholder={formData.db_role === 'replica' ? t('phConnNameReplica') : t('phConnName')}
                  required
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('fSlug')}</label>
                <input
                  type="text"
                  value={formData.slug}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('fDbName')}</label>
                <input
                  type="text"
                  value={formData.db_name}
                  onChange={(e) => setFormData({ ...formData, db_name: e.target.value })}
                  className="input-base w-full"
                  placeholder={t('phDbName')}
                  required
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('fHost')}</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('fPort')}</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('fUsername')}</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('fPassword')}</label>
                <input
                  type="password"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('fMaxConn')}</label>
                <input
                  type="number"
                ...
                <p className="text-xs text-gray-400 mt-1">
                  {t('maxConnHintAdd', { cap: TENANT_MAX_CONNECTIONS_CAP })}
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('fConnTimeout')}</label>
                <input
                  type="number"
                ...
                <div className="flex items-start space-x-2">
                  ...
                  <div className="flex-1">
                    ...
                    {testResult.server_version && (
                      <p className="text-xs text-green-600 mt-1">{t('serverVersion', { v: testResult.server_version })}</p>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="flex items-center space-x-3 pt-4 border-t">
              <button type="button" onClick={handleTestConnection} disabled={testingConnection} className="btn-default">
                ...
                {testingConnection ? t('testing') : t('testConn')}
              </button>
      ...
      <div className="space-y-4">
        {loading && userConnections.length === 0 ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 mb-4">{t('emptyList')}</p>
                ...
                <div className={`card p-5 ${currentConnection?.database_id === conn.database_id ? 'ring-2 ring-blue-500' : ''}`}>
                  <div className="flex items-start justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          {conn.is_primary && (
                            <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">
                              <i className="fas fa-star text-xs mr-1"></i>{t('badgeDefault')}
                            </span>
                          )}
                        </div>
                        <p className="text-xs text-gray-500 mt-0.5">{conn.db_host}:{conn.db_port}/{conn.db_name}</p>
                        <p className="text-xs text-gray-600 mt-1">
                          {t('poolCount', { n: conn.max_connections ?? DEFAULT_TENANT_MAX_CONNECTIONS })}
                          {' · '}
                          {t('acquireTimeout', { n: conn.connection_timeout ?? DEFAULT_TENANT_ACQUIRE_TIMEOUT_SECS })}
                        </p>
                        {replicas.length > 0 && (
                          <p className="text-xs text-green-600 mt-1">
                            <i className="fas fa-project-diagram mr-1"></i>
                            {t('replicaCount', { n: replicas.length })}
                          </p>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {updatingId === conn.database_id ? t('updating') : t('edit')}
                      </button>
                      <button
                      ...
                      >
                        ...
                        {currentConnection?.database_id === conn.database_id ? t('current') : t('use')}
                      </button>
                      <button
                      ...
                      <div key={replica.database_id} className="card p-4 bg-gray-50">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs text-green-600">
                            <i className="fas fa-circle text-xs mr-1"></i>{t('readonly')}
                          </span>
                        </div>
                      </div>
                    ))}
          ...
          >
            ...
            <form onSubmit={handleEditSave} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('fSlugEdit')}</label>
                  <input
                    type="text"
                    value={editForm.slug}
                  ...
                  <p className="text-[11px] text-amber-600 mt-1">
                    <i className="fas fa-exclamation-triangle mr-1"></i>{t('slugWarn')}
                  </p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('fDbName')}</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('fHostReq')}</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('fPortReq')}</label>
                  <input
                    type="number"
                ...
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('fPasswordEdit')}</label>
                  <input
                    type="password"
                    value={editForm.db_password}
                    onChange={(e) => setEditForm({ ...editForm, db_password: e.target.value })}
                    className="input-base w-full"
                    placeholder={t('phPwdKeep')}
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-400 mt-1">
                    {t('maxConnHintEdit', { cap: TENANT_MAX_CONNECTIONS_CAP })}
                  </p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('fConnTimeout')}</label>
                  ...
                  <div className="flex items-start space-x-2">
                    ...
                    <div className="flex-1">
                      ...
                      {editTestResult.server_version && (
                        <p className="text-xs text-green-600 mt-1">{t('serverVersion', { v: editTestResult.server_version })}</p>
                      )}
                    </div>
                  </div>
                </div>
              ...
              <div className="flex items-center space-x-3 pt-4 border-t">
                <button type="button" onClick={handleEditTest} disabled={editTesting} className="btn-default">
                  ...
                  {editTesting ? t('testing') : t('testConn')}
                </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div>
                <h2 className="text-lg font-semibold text-gray-900">{t('poolSettings')}</h2>
