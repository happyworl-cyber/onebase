...
function RpcAclPageInner() {
  ...
  // 同名重载（PostgreSQL 允许函数同名不同签名，pg_proc 会返回多行）。
  // 后端 ACL 是按 `${schema}.${function_name}` 存的，**不带签名**——也就是
  // 同名所有重载共享同一条 ACL。下拉里按"完整签名"展开两行既会触发 React
  // 重复 key 警告（key 就是 `${schema}.${function_name}`），也会让人误以
  // 为能精确授权到某个重载。这里直接按名字去重，多于一个重载时附加文案
  // 提示"还有 N 个重载共享同一授权"。
  const uniqueFunctions = useMemo(() => {
    const seen = new Map<string, FunctionInfo & { overloadCount: number }>()
    for (const f of functions) {
      const key = `${f.schema_name}.${f.function_name}`
      const prev = seen.get(key)
      if (prev) {
        prev.overloadCount += 1
      } else {
        seen.set(key, { ...f, overloadCount: 1 })
      }
    }
