...
/**
 * `/dashboard/es-connections` —— 当前项目下的 Elasticsearch 反向代理管理。
 *
 * 视图分两层：
 *   1. **连接列表**（左 / 上）：列出当前租户的所有 ES 集群配置；点击行进入详情
 *   2. **连接详情**（右 / 下）：编辑连接 + 管理代理 token + 显示业务端接入指南
 *
 * 关键 UX：
 *   - 凭据明文（ApiKey / basic 密码）**仅在创建/更新表单提交瞬间**经过前端，
 *     后续永远拿不回；前端从不缓存。
 *   - 代理 token 明文（cres_es_xxx）**只在创建成功一次性弹窗**显示，关闭即丢。
...
export default function EsConnectionsPage() {
  ...
  return <EsConnectionsManager tenantId={currentTenant.id} />
}
...
function UsageTab({ connection }: { connection: EsConnection }) {
  ...
  const proxyBase = `${origin}/api/es`
  const appBase = `${origin}/api/es-app`
