 ...
 *
 * 调用形态与 Auto API 一致：项目 ID 直接出现在路径里，便于在迁移 Supabase
 * 代码时可视化地试跑：
 *   const { data } = await supabase.rpc('console_get_user_projects', { user_id: 1 })
 *
 * 直接对应到本项目的：
 *   POST /api/v1/{databaseId}/rpc/console_get_user_projects   { "user_id": 1 }
 *
 * 功能：
...
function buildCurl(opts: {
  ...
  const { method, fnName, schema, args, singleObject, baseURL, databaseId } = opts
  // 没选项目时 fallback 到占位符；实际调用按钮会校验，cURL 也只是给用户看的。
  const dbSeg = databaseId ?? '<DATABASE_ID>'
...
export default function RpcPlaygroundPage() {
  ...
  // 项目 ID（即 database_id）现在是 RPC 路径的必填段——取自当前项目或当前连接。
  // 两者其一存在即可：dashboard 入口默认设置 currentTenant，而切库/连接选择器走 currentConnection。
  const databaseId =
    (currentTenant?.database_id ?? currentConnection?.database_id) ?? null
  ...
  }, [fnName, schema, method, singleObject, parsedArgs, baseURL, databaseId])
  ...
  const runRpc = async () => {
    ...
    if (databaseId == null) {
      notify.warning('请先选择项目（database_id 缺失，无法拼接 RPC 路径）')
      return
    }
    if (!parsedArgs.ok) {
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            ...
            <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs ml-1">
              /api/v1/{databaseId ?? '{databaseId}'}/rpc/&lt;fn&gt;
            </code>
            ，与表 CRUD 同款 URL 形态。
          </p>
        </div>
