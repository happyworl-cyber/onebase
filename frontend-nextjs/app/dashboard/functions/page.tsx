...
interface FunctionInfo {
  ...
  source_code: string
  /**
   * 当前函数所属扩展（如 `citext`、`pg_trgm`）。NULL = 用户自建函数。
   * 用 pg_depend.deptype='e' 反查 pg_extension 得到——这是 PG 标准做法，
   * 单看 language='internal'/'c' 不够准（业务也能写 C 扩展函数）。
   */
...
export default function FunctionsPage() {
  ...
  // 加载函数列表。
  //
  // SQL 不做扩展过滤——只用 LEFT JOIN 把每个函数所属的 `extension_name`
  // 带出来（用户函数为 NULL，扩展函数为扩展名，如 'citext'）。是否隐藏
  // 由前端 toggle 决定，切换不必重查库。
