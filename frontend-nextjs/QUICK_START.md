...
访问：**http://localhost:3006**（`package.json` 中固定的 dev 端口）。

> 后端地址会按下面的顺序自动解析（在 `next.config.js` 里实现），通常**不用手动指定**：
>
> 1. 显式 `NEXT_PUBLIC_API_URL`（生产 / Docker 用）
> 2. 项目根目录 `../.env` 中的 `HOST` + `PORT`——和后端用同一份配置
> 3. 兜底 `http://127.0.0.1:3000`
>
> 启动时控制台会打印一行 `[next.config] backend URL = http://...`，可以核对。
>
> 如果想临时指向别的后端，仍然支持：
> ```bash
> NEXT_PUBLIC_API_URL=http://other-host:3010 npm run dev
> ```
...
后端默认运行在 `http://127.0.0.1:3000`，但如果你在根目录 `.env` 改了 `PORT`（例如 3010），
`next.config.js` 会自动跟随，无需重复配置。
...
推荐方式（自动生成正确的 bcrypt 哈希并写入）：

```bash
cargo run --bin create_admin
```

也可直接执行 SQL（密码同样为 `Admin123`）：
